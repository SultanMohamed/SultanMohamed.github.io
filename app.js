...
bmiForm.addEventListener('submit', e => {
    ...
    if (BMI.toFixed(1) >= 30) {
        ...
        bmiResult.querySelector('span.weight-plan').textContent = `Your BMI suggests that you may be at risk for a range of health problems, including heart disease, diabetes, and certain cancers. Consult with a healthcare professional to determine a weight loss plan.`;

    } else if (BMI.toFixed(1) > 25) {
        bmiResult.querySelector('span.category').textContent = 'Overweight: ';

        bmiResult.querySelector('span.weight-plan').textContent = `Your BMI suggests that you may be at risk for health problems, including heart disease and diabetes. Consider making healthy lifestyle changes to achieve a healthy weight.`;

    } else if (BMI.toFixed(1) >= 18.5) {
        bmiResult.querySelector('span.category').textContent = 'Normal weight: ';

        bmiResult.querySelector('span.weight-plan').textContent = `Congratulations! Your BMI is within the healthy range of 18.5 to 24.9. Keep up your healthy habits to maintain your weight and support your overall well-being.`;

    } else if (BMI.toFixed(1) < 18.5) {
        bmiResult.querySelector('span.category').textContent = 'Underweight: ';

        bmiResult.querySelector('span.weight-plan').textContent = `You may be at risk of nutritional deficiencies and other health problems. Consult with a healthcare professional to determine a healthy weight gain plan.`;
    }
...
tipCalculatorForm.addEventListener('submit', e => {
    ...
    if (billAmount === '' || service === 0) {
        ...
        document.querySelector('div.error').innerHTML = 'Ayaa! Please enter some values for the bill amount and the service quality!';
    ...
    messageForBill.textContent = `Thank you for using our tip calculator! Based on your inputs, the tip amount is $${tipAmount.toFixed(2)} each and the total tip is $${tipTotal.toFixed(2)}. Please remember to pay the total amount of $${totalAmount}, which includes the bill and the tip, to show appreciation for the service you received. We hope you had a great experience and look forward to seeing you again!`;
});
...
tempConversionForm.addEventListener('submit', e => {
    ...
    if (convertTempFrom === 'F' && convertTempTo === 'C') {
        ...
        tempConversionResult.textContent = `${temperature}°F = ${celsius.toFixed(2)}°C`;
    } else if (convertTempFrom === 'F' && convertTempTo === 'K') {
        kelvin = (temperature - 32) * 5 / 9 + 273.15;
        tempConversionResult.textContent = `${temperature}°F = ${kelvin.toFixed(2)}K`;
    }

    // celsius to fahrenheit and kelvin
    if (convertTempFrom === 'C' && convertTempTo === 'F') {
        fahrenheit = temperature * 9 / 5 + 32;
        tempConversionResult.textContent = `${temperature}°C = ${fahrenheit.toFixed(2)}°F`;
    } else if (convertTempFrom === 'C' && convertTempTo === 'K') {
        kelvin = Number(temperature) + 273.15;
        tempConversionResult.textContent = `${temperature}°C = ${kelvin.toFixed(2)}K`;
    }

    // kelvin to fahrenheit and celsius
    if (convertTempFrom === 'K' && convertTempTo === 'F') {
        fahrenheit = (temperature - 273.15) * 9 / 5 + 32;
        tempConversionResult.textContent = `${temperature}K = ${fahrenheit.toFixed(2)}°F`;
    } else if (convertTempFrom === 'K' && convertTempTo === 'C') {
        celsius = temperature - 273.15;
        tempConversionResult.textContent = `${temperature}K = ${celsius.toFixed(2)}°C`;
    }
...
quizForm.addEventListener('submit', e => {
    ...
    // Get user answers
    const userAnswers = [quizForm.qn1.value, quizForm.qn2.value, quizForm.qn3.value, quizForm.qn4.value, quizForm.qn5.value];
    ...
    const divTop = scrollToResults.getBoundingClientRect().top + window.pageYOffset;
